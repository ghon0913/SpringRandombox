		...
		<table class="table">
			...
			<c:if test="${! empty pagedto.glist }">
				...
				<c:forEach var="goods" items="${pagedto.glist }">
					<tr>
						<td><a href="goodsinforetrieve.do?gCode=${goods.gCode }"
							id="gCode">${goods.gCode }</a></td>
						<td>${goods.gImage }${goods.gName }</td>
						<td>${goods.gCategory }</td>
						<td>${goods.gPrice }</td>
						<td><a href="questionList?gCode=${goods.gCode}">답변하러가기</a></td>
					</tr>
