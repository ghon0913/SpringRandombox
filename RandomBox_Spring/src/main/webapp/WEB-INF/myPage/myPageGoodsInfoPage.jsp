...
<c:if test="${endPage > 1 }">
	...
	<c:if test="${curPage != 1 }">
		<a href="goodsinfo?curPage=1"> 처음 </a>
	</c:if> &nbsp;
...
<!-- 이전페이지블럭 -->
	...
	<c:if test="${ startPage!=1}">
		<a href="goodsinfo?curPage=${startPage-1}"> &lt;&lt; </a>
	</c:if> &nbsp;
...
<!-- 이전페이지로 이동 -->
	...
	<c:if test="${curPage!=1}">
		<a href="goodsinfo?curPage=${curPage-1 }"> &lt; </a>
	</c:if> &nbsp;
...
<!-- 블럭당 표시할 페이지번호들 -->
	...
	${status.index }
		...
		<c:if test="${curPage!=status.index }">
			<a href="goodsinfo?curPage=${status.index }"> ${status.index } </a>
		</c:if> &nbsp;
</c:forEach>
	...
	<c:if test="${curPage!=endPage }">
		<a href="goodsinfo?curPage=${endPage }"> ${endPage } </a>
	</c:if>
...
<!-- 다음페이지블럭 -->
	...
	<c:if test="${endPage!=totalNum}">
		<a href="goodsinfo?curPage=${ endPage+1}"> &gt;&gt; </a>
	</c:if> &nbsp;
...
<!-- 마지막페이지로이동 -->
	...
	<c:if test="${curPage !=totalNum}">
		<a href="goodsinfo?curPage=${endPage+1 }"> 마지막 </a>
	</c:if>
</c:if>
&nbsp;
